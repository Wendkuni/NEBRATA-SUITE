...
export class SaisieBrouillonParContribuableComponent extends TransitionDomaineMorcellementComponent implements OnInit {
  ...
  constructor( public dialog: MatDialog,
    ...
    super(router, dialog,_snackBar, confirmService, _adapter, mediaObserver, fb, categoriePieceService, morcellementService, acteurService,
      contribuableService, structureService, parcelleService, mandatService);
  ...
  ngOnInit(): void {
    ...
    this.mandatChoisie = this.morcellement.mandats[0];

    this.guidProprietaire=this.authentificatedUser.guid;
    if (this.morcellement.mandats) {
      this.mandatRemoteAutocomplete.customNgSelectConfig.listRessource$ = of(this.morcellement.mandats);
      this.mandatRemoteAutocomplete.initialList = this.morcellement.mandats;
    }
    this.parcelleChoisie=this.morcellement.parcelle;
    this.formulaire.patchValue({
      numero: this.morcellement.numero,
      action: this.transition.code,
      mandat: this.morcellement?.mandats[0]?.id,
      dossier: {
