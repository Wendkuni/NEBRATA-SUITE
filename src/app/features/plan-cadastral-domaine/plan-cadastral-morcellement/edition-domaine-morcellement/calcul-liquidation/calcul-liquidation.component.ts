...
@Component({
  selector: 'app-morcellement-calcul-liquidation',
  templateUrl: './calcul-liquidation.component.html',
  styleUrls: ['./calcul-liquidation.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class CalculLiquidationComponent extends TransitionDomaineMorcellementComponent implements OnInit {
  ...
  if(this.morcellement.titresRecette && this.morcellement.titresRecette.length>0) {
    titreRecette=this.morcellement.titresRecette[0];
  }
 ...
 if(titreRecette && titreRecette.impots) {
  ...
  onSubmit() {
    ...
    this.morcellementService.executer(this.formulaire.value).subscribe(data => {
