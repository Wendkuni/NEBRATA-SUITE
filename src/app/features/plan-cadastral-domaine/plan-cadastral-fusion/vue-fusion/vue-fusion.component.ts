...
@Component({
  selector: 'app-vue-fusionnement',
  templateUrl: './vue-fusion.component.html',
  styleUrls: ['./vue-fusion.component.scss'],
  animations: [
...
export class VueFusionnementComponent implements OnInit {
  ...
  get urlAction(){
    return `${environment.FRONTEND_ROUTES.PROCESSUS_PLAN_CADASTRAL_FUSION}/edition`;
  }
  ...
  ngOnInit(): void {
   ...
   // console.log(this.fusion);
    this.dataSourcePiece.data = this.fusion.listPieces;
    this.dataSourceDocument.data = this.fusion.documents;
    this.dataSourceParcelles.data=this.fusion.parcelles;
    this.dataSourceMandat.data= this.fusion.mandats;
 
  
      if( this.fusion.titresRecette) {
        
        this.fusion.titresRecette.forEach(titre => {
        ...
        let arrayOrArrayExonertaion =  this.fusion.titresRecette.map(titre =>titre.impots.map(impot=>impot.exonerations));
  ...
  public get document(): Document{
    return (this.fusion.documents.length>0) ?this.fusion.documents[0]:null;
  }
}
