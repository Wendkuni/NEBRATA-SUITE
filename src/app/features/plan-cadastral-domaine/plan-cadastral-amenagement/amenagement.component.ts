...
@Component({
  selector: 'app-plan-cadastral-amenagement',
  templateUrl: './amenagement.component.html',
  styleUrls: ['./amenagement.component.scss']
})
export class AmenagementComponent extends AbstractSycadTableComponent  implements OnInit {
  ...
  constructor( private _snackBar: MatSnackBar,
               ...
               private injector: Injector, public planCadastralService: AmenagementService)
  { super();}

  ngOnInit(): void {


    this.monTableau=new SycadTableMetaData("Tableau des dossiers d'aménagement'",true,false);
    this.monTableau
    ...
    .pushColumn(new SycadTableColonne("destination.libelle","Destination",true,TypeColonne.STRING,true,""))
      ...
      this.monTableau.typeRessource="DossierAmenagementResource";
  ...
  public async loadComponent() {
    ...
    tableauComponent.instance.onPreviewData.subscribe($event =>  this.vueAmenagement($event));
    tableauComponent.instance.refeshDataSubject=this.refeshDataSubject;
  }
  public finishExport(event: StatsExport) {
    this._snackBar.open(`Export Plan cadastral aménagement  terminé. Fichier : ${event.fileName}, Taille : ${event.size}, Chargement : ${event.time} seconde(s) `, 'Ok', {
      duration: 4000
    });
  ...
  public actionProcessus(context: ActionProcessusEvent) {
    if(context.numero){
      this.router.navigate([`${environment.FRONTEND_ROUTES.PROCESSUS_PLAN_CADASTRAL_AMENAGEMENT}/edition`, context.numero,context.transition.code],{
        state: {
          context
        }
      });
    }else {
      this.router.navigate([`${environment.FRONTEND_ROUTES.PROCESSUS_PLAN_CADASTRAL_AMENAGEMENT}/edition`]);

    }

  }

  public vueAmenagement(amenagement: PlanCadastralAmenagementElement){
     this.router.navigate([`${environment.FRONTEND_ROUTES.PROCESSUS_PLAN_CADASTRAL_AMENAGEMENT}/view`, amenagement.numero]);
   }
