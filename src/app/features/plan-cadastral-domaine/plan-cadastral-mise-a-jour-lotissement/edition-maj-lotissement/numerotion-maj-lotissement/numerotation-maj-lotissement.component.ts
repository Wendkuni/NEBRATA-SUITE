...
import { TransitionPlanCadastralMiseAJourLotissementComponent } from '../transition-maj-lotissement.component';
import { PlanCadastralMiseAjourLotissementService } from '@sycadApp/services/workflow/common/maj-lotissement.service';

@Component({
  selector: 'app-numerotation-maj-lotissement',
  templateUrl: './numerotation-maj-lotissement.component.html',
  styleUrls: ['./numerotation-maj-lotissement.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class NumerotationMajLotissementComponent extends TransitionPlanCadastralMiseAJourLotissementComponent implements OnInit {
...
ngOnInit(): void {
  this.formulaire.patchValue({
   ...
   if(this.majLotissement?.parcellesAAjouter) {
  this.majLotissement.parcellesAAjouter.map((parcelle) => {
...
onSubmit() {
  ...
  let { action, numero, parcellesAAjouter, ilotsAAjouter, sectionsAAjouter } = this.formulaire.value;
  let { observation } = this.formulaire.value.dossier;
  let dossier = { action, numero, observation, parcellesAAjouter, ilotsAAjouter, sectionsAAjouter };

  this.majLotissmentService.executer(dossier).subscribe(data => {
    this.loadingEvent.emit(false);
    this.openSnackBar('Dossier mise a jour lotissement modifié avec succès', 'OK');
