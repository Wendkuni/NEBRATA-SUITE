...
@Component({
  selector: 'app-degre-successoral',
  templateUrl: './degre-successoral.component.html',
  styleUrls: ['./degre-successoral.component.scss']
})
export class DegreSuccessoralComponent extends AbstractSycadTableComponent implements OnInit {
  ...
  constructor(private _snackBar: MatSnackBar,
              ...
              private injector: Injector,public degreSuccessoralService: DegreSuccessoralService)
  {super(); }
  public async loadComponent() {
    ...
    tableauComponent.instance.backendApiService=this.degreSuccessoralService;
  ...
  ngOnInit(): void {
    this.monTableau=new SycadTableMetaData('Tableau des degrés successoral',true,false);
    this.monTableau

      .pushColumn(new SycadTableColonne("libelle","Libelle",true,TypeColonne.STRING,true,""))
      .pushColumn(new SycadTableColonne("rang","Rang",true,TypeColonne.STRING,true,""));
    this.monTableau.typeRessource="DegreSuccessoralResource";
    this.loadComponent();
  }
  public openFormModal(degreSuccessoral: DegreSuccessoral){
    let { width,height,position}=this.getCorrectWidth();
    this.dialogRef = this.dialog.open(FormDegreSuccessoralComponent, {
      data: degreSuccessoral,
    ...
    this.dialogRef.afterClosed().subscribe(documentType => {
      if(documentType) {
  ...
  public dialogRef: MatDialogRef<FormDegreSuccessoralComponent,any>;
  ...
  public modifier(degreSuccessoral) {
    this.openFormModal(degreSuccessoral);
  }
  public supprimer(data) {
    this.degreSuccessoralService.delete(data.id).subscribe(
      data => {
