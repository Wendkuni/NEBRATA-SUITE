...
export class RegionsComponent extends AbstractSycadTableComponent implements OnInit {
  ...
  constructor(  private _snackBar: MatSnackBar,
                ...
                private injector: Injector, public regionsService : RegionsService) {
    super();
   }
   ...
   public async loadComponent() {
    ...
    tableauComponent.instance.backendApiService=this.regionsService;
  ...
  ngOnInit():void {
    this.monTableau=new SycadTableMetaData("Tableau des régions",true,false);
    this.monTableau
     ...
     this.monTableau.typeRessource="RegionResource";
    ...
    public openFormModal(region: RegionElement){
     ...
     this.dialogRef.afterClosed().subscribe(region => {
       if(region){
        if(region.id) {
          this.openSnackBar("La région est modifiée avec succès","OK");   
        }else {
         this.openSnackBar("La région est ajoutée avec succès","OK");
        }
       }
  ...
  public dialogRef: MatDialogRef<FormRegionComponent,any>;
...
public modifier(region) {
 this.openFormModal(region);
}
public supprimer(data) {
 this.regionsService.delete(data.id).subscribe(
   data => {
     this.openSnackBar("La région est supprimée avec succès","OK");
       this.refeshDataSubject.next("");
   },
   errorResponse => {
    ...
    //// console.log(errorData)
     ...
     this.openSnackBar("Impossible de supprimer la région","OK");
   }
 );
}
