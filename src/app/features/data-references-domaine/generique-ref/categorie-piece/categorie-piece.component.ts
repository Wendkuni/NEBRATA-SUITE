...
import { FormCategoriePieceComponent} from '@sycadFeature/data-references-domaine/generique-ref/categorie-piece/form-categorie-piece/form-categorie-piece.component';
...
@Component({
  ...
  styleUrls: ['./categorie-piece.component.scss']
})
export class CategoriePieceComponent extends AbstractSycadTableComponent implements OnInit {
  @ViewChild("datatable1", { read: ViewContainerRef })
  private anchor: ViewContainerRef;

  @ViewChild("templateCategoriePiece") tpl: TemplateRef<any>;
  ...
  public async loadComponent() {
    ...
    tableauComponent.instance.backendApiService= this.categoriePieceService;
  ...
  ngOnInit(): void {
    this.monTableau=new SycadTableMetaData("Tableau des catégories de pièces",true,false);
    this.monTableau.isExpand=true;
    this.monTableau.typeRessource="CategoriePieceResource";
    this.monTableau
  ...
  public openFormModal(categoriePiece: CategoriePiece){
    let { width,height,position}=this.getCorrectWidth();

    this.dialogRef = this.dialog.open(FormCategoriePieceComponent, {
      data: categoriePiece,
    ...
    this.dialogRef.afterClosed().subscribe(categoriePiece => {
      if(categoriePiece) {
  ...
  public dialogRef: MatDialogRef<FormCategoriePieceComponent,any>;
  ...
  public modifier(categoriePiece) {
    this.openFormModal(categoriePiece);
  }
  public supprimer(data) {
    this.categoriePieceService.delete(data.id).subscribe(
      data => {
