...
@Component({
  selector: 'app-type-transition',
  templateUrl: './type-transition.component.html',
  styleUrls: ['./type-transition.component.scss']
})
export class TypeTransitionComponent extends AbstractSycadTableComponent implements OnInit {
  ...
  constructor(private _snackBar: MatSnackBar,
              ...
              private injector: Injector,public typeTransitionService: TypeTransitionService)
  {super(); }
  public async loadComponent() {
    ...
    tableauComponent.instance.backendApiService=this.typeTransitionService;
  ...
  public finishExport(event: StatsExport) {
    this._snackBar.open(`Export type de transition terminée. Fichier : ${event.fileName}, Taille : ${event.size}, Chargement : ${event.time} seconde(s) `, 'Ok', {
      duration: 4000
    });
  ...
  ngOnInit(): void {
    ...
    this.monTableau.typeRessource="TypeTransitionResource";
  ...
  public ajout() {
    this.router.navigate([`${environment.FRONTEND_ROUTES.CONFIGURATION_TYPE_TRANSITION}/edition`]);
  }
  public modifier(typeTransition) {
    this.router.navigate([`${environment.FRONTEND_ROUTES.CONFIGURATION_TYPE_TRANSITION}/edition`, typeTransition.id]);
  }
  public supprimer(data) {
    this.typeTransitionService.delete(data.id).subscribe(
      data => {
