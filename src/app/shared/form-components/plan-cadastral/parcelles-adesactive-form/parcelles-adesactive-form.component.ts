...
import {AbstractControl, ControlValueAccessor, FormArray, NG_VALUE_ACCESSOR} from '@angular/forms';
...
@Component({
  selector: 'app-parcelles-adesactive-form',
  templateUrl: './parcelles-adesactive-form.component.html',
  styleUrls: ['./parcelles-adesactive-form.component.scss'],
  providers: [{ provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => ParcellesADesactiveFormComponent), multi: true }],
  encapsulation: ViewEncapsulation.None,
})
export class ParcellesADesactiveFormComponent implements ControlValueAccessor, OnInit, OnDestroy, OnChanges {
  ...
  public displayedColumns: string[] = ['numero', 'numeroAncien', 'libelle', 'superficie', 'destination', 'action'];
  ...
  public onChangeParcelle(parcelles) {
    ...
    this.lParcelles = this.lParcelles.concat(parcelles);
    this.lParcelles = this.getUnique(this.lParcelles, 'id');
    this.dataSourceParcelles = this.lParcelles;
    this.onChange(this.lParcelles.map(parcelle => parcelle.id));
    this.change.emit(this.lParcelles);
  }
  ...
  writeValue(id: null | Number[]): void {
    ...
    //// console.log("writeValue ", this.parcelleChoisie);
    if (id === null) {
      this.communeId=null;
  ...
  supprimerParcelle(elm){
    this.lParcelles = this.lParcelles.filter(i => i !== elm);
    this.dataSourceParcelles = this.lParcelles;
    this.onChange(this.lParcelles.map(parcelle => parcelle.id));
    this.change.emit(this.lParcelles);
  }
