...
@Component({
  selector: 'app-sd-section-aajouter-form',
  templateUrl: './sd-section-aajouter-form.component.html',
  styleUrls: ['./sd-section-aajouter-form.component.scss'],
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => SdSectionAajouterFormComponent),
    multi: true
  }],
})
export class SdSectionAajouterFormComponent implements ControlValueAccessor, OnInit, OnDestroy, OnChanges {
  @Input()
  touched: boolean;
  ...
  public displayedColumns: string[] = ['commune', 'numero', 'numeroAncien', 'action'];
  ...
  ngOnInit(): void {
    ...
    if (this.sectionsChoisie && this.sectionsChoisie.length > 0) {
      ...
      this.totalElements = this.sectionAAjouter.length;
    }
  }
  ...
  updateDataSource(): void {
    ...
    this.dataSourceSection.data = this.sectionAAjouter.value.slice(startIndex, endIndex);
  }
