...
@Component({
  selector: 'app-envoyer-avec-observation-dossier-transition',
  templateUrl: './envoyer-avec-observation-dossier-transition.component.html',
  styleUrls: ['./envoyer-avec-observation-dossier-transition.component.scss']
})
export class EnvoyerAvecObservationDossierTransitionComponent  extends TransitionComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.processusService.executer(dataPost).subscribe(data => {
      this.loadingEvent.emit(false);
        this.openSnackBar('Le traitement a été fait avec succès', 'OK');
        this.router.navigate([this.frontendUrl]);
      },
      errorResponse => {
        this.loadingEvent.emit(false);
